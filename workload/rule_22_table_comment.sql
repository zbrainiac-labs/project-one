...
-- Single-line regex: (?i)^(?!\s*--)\s*CREATE\s+(OR\s+REPLACE\s+)?TABLE\s+(?!.*\bCOMMENT\b).*$
-- Multiline regex: (?is)CREATE\s+(?:OR\s+REPLACE\s+)?TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?\S+(?:(?!\bCOMMENT\b).){5,}(?:AS\s+SELECT|\(\s*\w)
-- =============================================================================
